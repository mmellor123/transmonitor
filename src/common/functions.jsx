...
export const Item = ({ title, to, icon, location}) => {
  ...
  return (
    <MenuItem
      ...
      icon={icon}
      component={<Link to={to} onClick={to}/>}
    >
...
export async function authenticateToken(token){
  const response = await fetch(BASE_URL+'/authenticate', {headers:{ 'Authorization':  'Bearer ' + token}}).then(res => res.ok).then((results) => {return results});
  return response;
}

//TODO make token refresh if access token has expired. Call authenticate Token, 
export async function refreshToken(token){
  const response = await fetch(BASE_URL+'/token', { method: "POST", body: JSON.stringify({token: token}), headers: { 'Content-Type':  "application/json"} })
   .then(res => res)
   .then(
    (results) => {
      if(results.ok){
...
export async function postData(url, payload, method, token) {
